class Boid {
    ...
    separate = (boids) => {
        ...
        for (let boid of boids) {
            ...
            if (d !== 0 && d < this.PERSEPTION_RADIUS / 2) {
                let diff = this.p5.createVector(this.position.x, this.position.y).sub(boid.position);  // static method p5.Vector.sub()
                diff.div(d);
                average.add(diff);
                totalBoids++;
            }
    ...
    show = () => {
        ...
        this.p5.rotate(this.velocity.x, this.velocity.y);  // why drawing like fish...
